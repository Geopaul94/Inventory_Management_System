...
class CustomersBloc extends Bloc<CustomersEvent, CustomersState> {
  ...
  List<CustomerDetailsModel> customersList = await FirestoreServiceCustomer().fetchAllCustomers();
  ...
  customersList.sort((a, b) => b.createdAt.compareTo(a.createdAt));
   ...
   emit(FetchCustomersInitialState(customerDetails: customersList));
    ...
    emit(FetchCustomersErrorState(Error: error.toString())); // Emit error state
  }
}
  ...
  Future<void> _onUpdateCustomerButtonClick(
      OnUpdateCustomerButtonClikEvent event, Emitter<CustomersState> emit) async {
    emit(CustomersLoadingState());
    try {
  ...
  Future<void> _onDeleteCustomerButtonClick(
      OnDeleteCustomerButtonClikEvent event, Emitter<CustomersState> emit) async {
    emit(CustomersLoadingState());
    try {
